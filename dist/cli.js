...
async function loadConfig(configPath) {
  ...
  const doc = (0, import_yaml.parse)(raw);
  ...
  const plugin = (doc.plugins ?? []).find((p) => {
    const src = p?.source ?? "";
    return src.includes("content-internationalization") || src.includes("quartz-content-internationalization");
  });
  const opts = plugin?.options ?? {};
  const rawLocales = Array.isArray(opts.locales) ? opts.locales : [];
  const locales = rawLocales.filter((l) => typeof l === "string");
